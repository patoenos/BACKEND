...
class Contenedor {  //recibe el archivo para el FS y despues se instancia con el archivo que le pasemos
    constructor(file) {
        this.file = file;
    }
// De regreso al async por los métodos del FS
    ...
    async save(object) {        
        try {
            ...
            return object.id;
        } catch (err) { // Crea el archivo si no existee            
            if (err.code === 'ENOENT') {
                object.id = 1;
